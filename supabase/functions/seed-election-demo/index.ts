...
const OBSERVER_ROLES = ['domestic_observer', 'international_observer', 'party_agent', 'media', 'election_official', 'security_personnel'];
...
serve(async (req) => {
  ...
  try {
    ...
    // Seed incident categories first
    for (const cat of INCIDENT_CATEGORIES) {
      await supabase.from('election_incident_categories').upsert({
        name: cat.name,
    ...
    const electionScenarios = [
      { country: AFRICAN_COUNTRIES[0], type: 'presidential', status: 'voting', daysOffset: 0, name: 'Presidential Election 2026' },
      { country: AFRICAN_COUNTRIES[1], type: 'gubernatorial', status: 'counting', daysOffset: -1, name: 'Gubernatorial Elections 2026' },
      { country: AFRICAN_COUNTRIES[2], type: 'parliamentary', status: 'scheduled', daysOffset: 30, name: 'Parliamentary Election 2026' },
      { country: AFRICAN_COUNTRIES[3], type: 'local', status: 'verification', daysOffset: -3, name: 'Municipal Elections 2026' },
      { country: AFRICAN_COUNTRIES[4], type: 'referendum', status: 'campaigning', daysOffset: 14, name: 'Constitutional Referendum 2026' },
      { country: AFRICAN_COUNTRIES[5], type: 'presidential', status: 'completed', daysOffset: -30, name: 'Presidential Election 2025' },
    ];
    ...
    for (const scenario of electionScenarios) {
      ...
      const { data: election, error } = await supabase.from('elections').insert({
        ...
        candidates: generateCandidates(scenario.type, scenario.country.name),
        political_parties: generateParties(scenario.country.name),
      }).select().single();
      ...
      if (election) {
        ...
        const stationCount = randomInt(15, 30);
        const stations: any[] = [];
        for (let i = 0; i < stationCount; i++) {
          const region = randomItem(scenario.country.regions);
          const { data: station } = await supabase.from('polling_stations').insert({
            ...
            latitude: -1.2921 + (Math.random() * 10 - 5),
            longitude: 36.8219 + (Math.random() * 20 - 10),
          }).select().single();
        ...
        // Create observers
        const observerCount = randomInt(20, 50);
        for (let i = 0; i < observerCount; i++) {
          ...
          const org = role === 'international_observer' ? randomItem(OBSERVER_ORGS.slice(0, 6)) : 
                      ...
                      role === 'party_agent' ? randomItem(generateParties(scenario.country.name)).name :
                      role === 'media' ? randomItem(['BBC', 'CNN', 'Al Jazeera', 'Reuters', 'AFP', 'Local Press']) :
                      'Government';
          
          await supabase.from('election_observers').insert({
            election_id: election.id,
            full_name: `[DEMO] Observer ${i + 1}`,
            email: `observer${i + 1}@demo.org`,
            ...
            assigned_stations: stations.slice(0, randomInt(1, 5)).map(s => s.id),
        ...
        if (['voting', 'counting', 'verification', 'completed'].includes(scenario.status)) {
          const incidentCount = randomInt(10, 40);
          for (let i = 0; i < incidentCount; i++) {
            const category = randomItem(INCIDENT_CATEGORIES);
            const severity = Math.random() > 0.7 ? randomItem(['critical', 'emergency']) : 
                            Math.random() > 0.5 ? category.severity_default : 
                            randomItem(['minor', 'moderate']);
            ...
            await supabase.from('election_incidents').insert({
              election_id: election.id,
              ...
              location_address: station?.station_name,
              latitude: station?.latitude || (-1.2921 + (Math.random() * 2 - 1)),
              longitude: station?.longitude || (36.8219 + (Math.random() * 2 - 1)),
              ...
              credibility_score: randomInt(40, 100),
        ...
        if (['verification', 'completed', 'counting'].includes(scenario.status)) {
          for (const station of stations.slice(0, Math.min(stations.length, 20))) {
            const registered = station.registered_voters;
            const turnout = Math.random() * 0.4 + 0.45; // 45-85% turnout
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Election demo data seeded successfully',
...
function generateCandidates(type: string, country: string): any[] {
...
function generateParties(country: string): any[] {
...
function generateIncidentDescription(category: string, country: string): string {
  const templates: Record<string, string[]> = {
    'Voter Intimidation': [
      'Group of individuals observed outside polling station making threatening gestures toward voters.',
      'Reports of voters being told who to vote for under threat of consequences.',
      'Armed individuals spotted near voting queue creating fear among voters.',
    ],
    'Ballot Irregularities': [
      'Pre-marked ballots discovered in ballot box during count verification.',
      'Significant discrepancy between number of voters recorded and ballots found.',
      'Damaged ballot papers making voter intent unclear.',
    ],
    'Polling Station Issues': [
      'Polling station opened 2 hours late due to delayed materials delivery.',
      'Biometric verification equipment malfunction causing long queues.',
      'Insufficient ballot papers requiring emergency resupply.',
    ],
    'Violence': [
      'Physical altercation between supporters of rival parties near polling station.',
      'Vandalism of campaign materials and election posters.',
      'Mob gathered outside causing disruption to voting process.',
    ],
    'Process Violations': [
      'Accredited observers denied entry to counting hall.',
      'Voters observed casting multiple ballots.',
      'Improper voter ID verification procedures followed.',
    ],
    'Campaign Violations': [
      'Active campaigning observed within restricted zone near polling station.',
      'Reports of voters being offered money or goods in exchange for votes.',
      'Unauthorized campaign materials distributed at polling location.',
    ],
    'Technical Issues': [
      'Power outage affecting electronic result transmission.',
      'Network connectivity issues preventing real-time reporting.',
      'Biometric system database synchronization failure.',
    ],
    'Media & Information': [
      'False results being circulated on social media before official announcement.',
      'Journalist equipment confiscated by security personnel.',
      'Coordinated disinformation campaign detected online.',
    ],
  };
